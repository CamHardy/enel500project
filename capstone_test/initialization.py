...
class Joints:
	...
	def move_joints(self, angles):
		...
		char = self.name[0]
		goal.trajectory.joint_names = ['head','neck','left_shoulder_tip','left_shoulder','left_upper_arm','left_elbow','right_shoulder_tip','right_shoulder','right_upper_arm','right_elbow']
	...
	def reset_speeds(self):
		...
		try:
			set_head_speed = rospy.ServiceProxy('/head_controller/set_speed', SetSpeed)
			set_head_speed(0.5)
		...
		try:
			set_head_speed = rospy.ServiceProxy('/neck_controller/set_speed', SetSpeed)
			set_head_speed(0.5)
		except rospy.ServiceException, e:
			print "Service call failed: %s"%e
...
def main():
	...
	robot.move_joints([6.277,1.566,2.804,-0.0138,-0.003,-0.2577,-0.007,0.05,0,-0.284])
	robot.reset_speeds()
