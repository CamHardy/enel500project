...
class HeadTracker():
    def __init__(self):
        ...
        self.head_pan_joint = "neck_controller"
        self.head_tilt_joint = "head_controller"
        ...
        self.max_pan = rospy.get_param("~max_pan", radians(177))
        self.min_pan = rospy.get_param("~min_pan", radians(0))
        self.max_tilt = rospy.get_param("~max_tilt", radians(350))
        self.min_tilt = rospy.get_param("~min_tilt", radians(241))
        ...
        self.pan_joint_state = JointState()
        rospy.Subscriber('/neck_controller/state', JointState, self.update_neck_joint_state, queue_size=1)

        # Wait until we actually have joint state values
        while self.pan_joint_state == JointState():
            rospy.sleep(1)

	self.tilt_joint_state = JointState()
	rospy.Subscriber('/head_controller/state', JointState, self.update_head_joint_state, queue_size=1)

        # Wait until we actually have joint state values
        while self.tilt_joint_state == JointState():
            rospy.sleep(1)
        ...
        rospy.loginfo("Waiting for Vision Telemetry and Joint topics.")

        rospy.wait_for_message('/head_controller/state', JointState)
	rospy.wait_for_message('/neck_controller/state', JointState)
        rospy.wait_for_message('/vision_telemetry', VisionCommand)

        # Subscribe to camera_info topics and set the callback
        self.image_width = 640
	self.image_height = 480
        
        # This might need to change to offset data
        #while self.image_width == 0 or self.image_height == 0:
        #    rospy.sleep(1)
            
        # Subscribe to vision command topic and set the callback
        self.vision_command_subscriber = rospy.Subscriber('/vision_telemetry', robot_msgs.VisionCommand, self.set_joint_cmd, queue_size=1)
    ...
    def set_joint_cmd(self, msg):
        ...
        try:
	    #maybe change the below to be the target_offset_x == 0 or target_offset_y == 0
	    ...
	    target_offset_x = msg.object.display_x
	    target_offset_y = msg.object.display_y            
	    ...
	    # might be an error due to array indexing
            current_pan = self.pan_joint_state.current_pos
            ...
            # Set the target position ahead or behind the current position
            current_tilt = self.tilt_joint_state.current_pos
    ...
    def init_servos(self):
        ...
        set_speed_service = '/' + self.head_pan_joint + '/set_speed'
	rospy.wait_for_service(set_speed_service)
	self.servo_speed[self.head_pan_joint] = rospy.ServiceProxy(set_speed_service, SetSpeed, persistent=True)

	set_speed_service = '/' + self.tilt_pan_joint + '/set_speed'
        rospy.wait_for_service(set_speed_service)
        self.servo_speed[head_tilt_joint] = rospy.ServiceProxy(set_speed_service, SetSpeed, persistent=True)
	...
	self.servo_speed[self.head_tilt_joint](self.default_joint_speed)
        ...
        self.servo_position[self.head_pan_joint] = rospy.Publisher('/' + self.head_pan_joint + '/command', Float64, queue_size=5)
	self.servo_position[self.head_tilt_joint] = rospy.Publisher('/' + self.head_tilt_joint + '/command', Float64, queue_size=5)            
    ...
    def update_neck_joint_state(self, msg):
        self.pan_joint_state = msg

    def update_head_joint_state(self, msg):
        self.tilt_joing_state = msg
    ...
    def shutdown(self):
        ...
        # Turn off updates from the vision command subscriber
        try:
            self.vision_command_subscriber.unregister()
        except:
            pass
